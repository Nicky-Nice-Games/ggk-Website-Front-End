  ...
  return (
    ...
    >
      ...
      <div className="flex w-full max-w-sm flex-col gap-6">
        <h1
          id="title"
          className="poppins font-bold text-center text-white text-4xl"
        >
          Gizmo Go-Kartz
        </h1>
        {<CurrentAuthForm setAccount={setAccount} />}
      </div>
    </div>
  );
};

const CurrentAuthForm = ({ setAccount }: { setAccount: Function }) => {
  ...
  switch (currentPage) {
    ...
    case "reset-password":
      return <PasswordResetPage onLoginClick={() => setCurrentPage("login")} />;
    default:
      return (
        <LoginForm
          onSubmit={(e) => {
          ...
          onResetPasswordClick={() => setCurrentPage("reset-password")}
        />
      );
  }
};
